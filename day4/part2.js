...
function findBestMinute(events, guard) {
    ...
    return { bestMinute: bestMinute, count: minutes[bestMinute] };
}

function findAllGuards(events) {
    const guards = [];
    for (const ev of events) {
        if (guards.indexOf(ev.guard) < 0)
            guards.push(ev.guard);
    }
    return guards;
}

function findMostFreqGuard(events, guards) {
    const guardMinutes = [];
    for (const guard of guards) {
        const b = findBestMinute(events, guard);
        b.bestGuard = guard;
        guardMinutes.push(b);
    }
    let best = -1;
    for (let i = 0 ; i< guardMinutes.length; i++) {
        if (best <0 || guardMinutes[i].count > guardMinutes[best].count)
            best = i;
    }
    return guardMinutes[best];
}

function findBestGuard(testInput, prefix) {
    const events = extractEvents(testInput);
    const guards = findAllGuards(events);
    const { bestGuard, bestMinute } = findMostFreqGuard(events, guards);
...
console.log("Expected: 99 * 45 = 4455");

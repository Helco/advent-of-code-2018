...
function advanceMap(map) {
    ...
    for (train of trains) {
        ...
        if (train.crashed)
            continue;
        for (other of trains) {
            if (other != train && !other.crashed && other.x == train.x && other.y == train.y) {
                other.crashed = true;
                train.crashed = true;
...
function findLastTrain(map) {
    let last = -1;
    for (let i = 0; i < map.trains.length; i++) {
        if (!map.trains[i].crashed) {
            if (last < 0)
                last = i;
            else
                return -1;
...
function getLastTrain(input) {
    var map = extractMap(input);
    var lastTrain = findLastTrain(map);
    while(lastTrain < 0) {
        advanceMap(map);
        lastTrain = findLastTrain(map);
    }
    return map.trains[lastTrain].x + "," + map.trains[lastTrain].y;
}
...
console.log("Expected: 6,4");
console.log("Actual:  " + getLastTrain(testInput));
console.log("Solution: " + getLastTrain(fs.readFileSync(__dirname + "/input", "utf8").split("\n")));